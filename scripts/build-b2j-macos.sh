...
# 用法：bash scripts/build-b2j-macos.sh [je2be-core 版本号，默认 0.12.4]
set -euo pipefail
...
echo ">> 下载 je2be-core v$VERSION 源码"
curl -L "https://github.com/kbinani/je2be-core/archive/refs/tags/v$VERSION.tar.gz" -o "$BUILD_DIR/je2be.tar.gz"
tar -xzf "$BUILD_DIR/je2be.tar.gz" -C "$BUILD_DIR"
SRC="$BUILD_DIR/je2be-core-$VERSION"

echo ">> CMake 配置（目标架构：${ARCH:-本机 $(uname -m)}）"
CMAKE_ARGS=(-S "$SRC" -B "$BUILD_DIR/build" -DCMAKE_BUILD_TYPE=Release -DJE2BE_BUILD_B2J=ON)
if [ -n "${ARCH:-}" ]; then
...
find "$BUILD_DIR/build" -type f \( -name 'libje2be*.dylib' -o -name 'libc++*.dylib' \) | while read -r lib; do
  cp "$lib" "$BACKENDS/" 2>/dev/null || true
done
chmod +x "$BACKENDS/b2j"
"$BACKENDS/b2j" --help >/dev/null 2>&1 || "$BACKENDS/b2j" 2>&1 | head -n3 || true
