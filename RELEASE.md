...
- 修复版本转换后主世界 `entities/` 与 `poi/` 目录可能丢失的问题，补充升级/降级回归测试。
- 新增“检查更新”按钮：从 GitHub Release 验签下载并安装最新版，安装完成后自动重启。
- 延续 2.0.0 的樱色与奶油白双栏界面，按“选择存档 → 设置版本 → 保存世界”引导操作；小窗口也能显示关键操作。
...
本地验证：20 项 Rust 单元测试、前端流程回归、Java 测试世界识别/转换/保存端到端，以及最小窗口布局、降级确认、取消后重试与解析失败恢复。原生文件对话框输入未完成自动化验证。
